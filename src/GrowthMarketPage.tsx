...
const tableGridClass = 'grid grid-cols-[40px_150px_80px_80px_100px_130px_260px_220px] gap-3'
const tableWidthClass = 'min-w-[1176px]'
...
function GrowthMarketPage() {
  ...
  const rows = useMemo(() => {
    ...
    const filtered = normalized
      ...
      : data.rows
    return [...filtered].sort((a, b) => b.position_52w_pct - a.position_52w_pct)
  }, [data, query])
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="rounded-[2rem] border border-white/80 bg-white/85 p-5 shadow-[0_24px_60px_rgba(15,23,42,0.06)] sm:p-7">
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <SummaryCard label="最终展示" value={data.summary.displayed_total} />
          </div>
        </motion.section>
        ...
        <section className="rounded-[2rem] border border-white/80 bg-white/82 p-4 shadow-[0_24px_60px_rgba(15,23,42,0.05)] sm:p-6">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">越接近 100%，当前价格越靠近 52 周高点</p>
            </div>
          ...
          <div className="mt-5 rounded-[1.5rem] border border-slate-200/80">
            <div ref={headerScrollRef} className="sticky top-0 z-30 overflow-hidden rounded-t-[1.5rem] border-b border-slate-200/90 bg-slate-50/95 shadow-[0_8px_20px_rgba(15,23,42,0.06)] backdrop-blur-xl">
              <div className={`${tableGridClass} ${tableWidthClass} items-center px-4 py-4 text-[11px] font-medium tracking-[0.12em] text-slate-400`}>
                <div>#</div><div>股票</div><div className="text-right">收盘价</div><div className="text-right">今日</div><div className="text-right">流通市值</div><div className="text-right">{formatPeriod(newerPeriod)}归母净利</div><div>主营业务</div><div className="text-center text-slate-600">52周内位置</div>
              </div>
            </div>
            <div
            ...
            >
              <div className={`${tableWidthClass} divide-y divide-slate-100 bg-white`}>
                {rows.map((row, index) => (
                  <div key={row.code} className={`${tableGridClass} items-center px-4 py-3.5 text-sm hover:bg-slate-50/80`}>
                    <div className="text-xs tabular-nums text-slate-300">{index + 1}</div>
                    <div><div className="font-medium text-slate-900">{row.name}</div><div className="mt-0.5 text-xs tracking-[0.08em] text-slate-400">{row.code}</div></div>
                    ...
                    <div className="rounded-[1rem] border border-sky-100 bg-sky-50/70 px-3 py-2.5">
